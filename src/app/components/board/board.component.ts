...
import { ICell } from 'src/app/components/board/cell.interface';
...
export class BoardComponent implements OnInit, OnDestroy {
    ...
    setBoard(): void {
        ...
        if (this.boardType === 'square') {
            ...
            for (let i = 0; i < this.boardLength; i++) {
                ...
                for (let j = 0; j < this.boardWidth; j++) {
                    let isActive: boolean =
                        Math.floor(this.boardLength / 2) === i ||
                        Math.floor(this.boardWidth / 2) === j ||
                        (this.boardLength % 2 === 0 &&
                            this.boardLength / 2 - 1 === i) ||
                        (this.boardWidth % 2 === 0 &&
                            this.boardWidth / 2 - 1 === j);
                    boardLine.push({
                        isBattleship: false,
                        leftBorder: false,
                        rightBorder: false,
            ...
            let longer: number =
                this.boardLength > this.boardWidth
                    ? this.boardLength
                    : this.boardWidth;
            let shorter: number =
                this.boardLength > this.boardWidth
            ...
            let cellsGap: number = Math.floor(
                (longer - shorter) / Math.floor(shorter / 2)
            );
            ...
            while (
                cellsGap -
                    (cellsNum - cellsGap * (Math.floor(shorter / 2) - 1)) >
                1
            )
                cellsGap--;
            ...
            for (
                let j = 1;
                j <= cellsNum - cellsGap * Math.floor(shorter / 2);
                j++
            )
                gaps[j]++;
            for (
                let j = 1;
                j <= cellsGap * Math.floor(shorter / 2) - cellsNum;
                j++
            )
                gaps[j]--;
            let edge =
                this.boardLength % 2 === 0 &&
                (this.boardLength <= 6 || this.boardWidth % 2 === 0)
                    ? 2
                    : 1;
            for (let i = 1; i <= Math.ceil(shorter / 2) - edge; i++) {
                ...
                for (let k = 1; k <= cellsNum; k++) {
                    ...
                    if (this.boardWidth % 2 === 0) {
                        if (Math.abs(this.boardLength - this.boardWidth) !== 1)
                            addToCol--;
                        this.board[Math.floor(this.boardLength / 2) + addToRow][
                            Math.floor(this.boardWidth / 2) - addToCol - 1
                        ].isActive = true;
                        this.board[Math.floor(this.boardLength / 2) - addToRow][
                            Math.floor(this.boardWidth / 2) - addToCol - 1
                        ].isActive = true;
                    ...
                    if (this.boardLength % 2 === 0) {
                        if (
                            Math.abs(this.boardLength - this.boardWidth) !==
                                1 &&
                            this.boardWidth % 2 !== 0
                        )
                            addToRow--;
                        this.board[
                            Math.floor(this.boardLength / 2) - addToRow - 1
                        ][Math.floor(this.boardWidth / 2) + addToCol].isActive =
                            true;
                        this.board[
                            Math.floor(this.boardLength / 2) - addToRow - 1
                        ][Math.floor(this.boardWidth / 2) - addToCol].isActive =
                    ...
                    this.board[Math.floor(this.boardLength / 2) - addToRow][
                        Math.floor(this.boardWidth / 2) + addToCol
                    ].isActive = true;
                    this.board[Math.floor(this.boardLength / 2) + addToRow][
                        Math.floor(this.boardWidth / 2) - addToCol
                    ].isActive = true;
    ...
    setIslands(): void {
        for (
            let k = 0;
            k < Math.floor((this.boardLength * this.boardWidth) / 10);
            k++
        ) {
            let i = 0,
                j = 0;
            ...
            while (!checkPosition) {
                ...
                if (!this.board[i][j].isActive || this.board[i][j].isIsland)
                    checkPosition = false;
            }
    ...
    setBattleships(): void {
        ...
        for (let k = this.battleshipNum; k >= 1; k--) {
            ...
            let length =
                this.boardType === 'rhombus'
                    ? this.getRandomInt(
                          1,
                          Math.min(
                              5,
                              Math.ceil(
                                  (this.boardLength * 0.9) /
                                      this.battleshipNum ** 0.5
                              ),
                              Math.ceil(
                                  (this.boardWidth * 0.9) /
                                      this.battleshipNum ** 0.5
                    ...
                    : this.getRandomInt(
                          ...
                          Math.min(
                              5,
                              (this.boardLength * this.boardWidth * 0.9) /
                                  this.battleshipNum
                          )
                      );
            let i = 0,
            ...
            while (!checkPosition) {
                ...
                for (let l = 0; l < length; l++) {
                    if (
                        ...
                        (j + l >= this.boardWidth ||
                            length + i > this.boardLength ||
                            !this.board[i][j + l].isActive ||
                            this.board[i][j + l].isBattleship ||
                            this.board[i][j + l].isIsland)
                    )
                        checkPosition = false;
                    else if (
                        direction === 'vertical' &&
                        (i + l >= this.boardLength ||
                            length + j > this.boardWidth ||
                            !this.board[i + l][j].isActive ||
                            this.board[i + l][j].isBattleship ||
                            this.board[i + l][j].isIsland)
                    )
                        checkPosition = false;
                }
            }
