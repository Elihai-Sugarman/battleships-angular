    ...
    implements AfterViewInit, OnInit, OnDestroy
{
    ...
    ngOnInit(): void {
        ...
        this.settingService.boardType$.subscribe(
            (value) => (this.boardType = value)
        );
    ...
    onSubmit(): void {
        ...
        else if (this.boardLength < this.minCells)
            this.boardLength = this.minCells;
        if (!this.boardWidth) this.boardWidth = 10;
        else if (this.boardWidth < this.minCells)
            this.boardWidth = this.minCells;
        if (!this.battleshipNum) this.battleshipNum = 10;
        if (
            this.battleshipNum >
            (this.boardLength * this.boardWidth) / this.battleshipNumBorder
        )
            this.battleshipNum = Math.floor(
                (this.boardLength * this.boardWidth) / this.battleshipNumBorder
            );
        this.settingService.setBoardSettings(
            this.boardLength,
            this.boardWidth,
            this.battleshipNum,
            this.createIslands
    ...
    onChangeType(type: 'square' | 'rhombus'): void {
        ...
        if (
            ...
            (this.boardLength < 3 ||
                this.boardWidth < 3 ||
                this.battleshipNum > (this.boardLength * this.boardWidth) / 4)
        )
            this.onSubmit();
    }
    ...
    handleInput(event: any) {
        ...
        if (
            (value < 2 && this.boardType === 'square') ||
            (value < 3 && this.boardType === 'rhombus')
        ) {
            this.error = true;
        } else this.error = false;
    }
